...
public class MenuRelatorios {
    ...
    private void pacientesHistorico() {
        gh.getPacientes().forEach(p -> {
            ...
            consultas.forEach(c -> System.out.println("     " + c.getDataHora().format(FMT)
                    + " | " + c.getMedico().getNome() + " | " + c.getStatus().getStatusFormatado()));
    ...
    private void consultasFiltro() {
        ...
        Predicate<Consulta> filtro = switch (f) {
            case 1 -> { sc.nextLine(); String cpf = Utilitario.lerString(sc, "CPF: "); yield c -> c.getPaciente().getCpf().equals(cpf); }
            case 2 -> { sc.nextLine(); String crm = Utilitario.lerString(sc, "CRM: "); yield c -> c.getMedico().getCrm().equals(crm); }
            case 3 -> { sc.nextLine(); String esp = Utilitario.lerString(sc, "Especialidade: ").toLowerCase(); yield c -> c.getMedico().getEspecialidade().toLowerCase().contains(esp); }
            default -> c -> true;
        };
        ...
        System.out.println("Futuras: " + futuras.size()); futuras.forEach(this::printConsulta);
        System.out.println("Passadas: " + passadas.size()); passadas.forEach(this::printConsulta);
    }

    // 4) Internações ativas
    private void internacoesAtivas() {
        var ativas = gh.getInternacoes().stream().filter(Internacao::estaAtiva).toList();
        if (ativas.isEmpty()) { System.out.println("Nenhuma internação ativa."); return; }
    ...
    private void estatisticasGerais() {
        ...
        porMedico.entrySet().stream().max(Map.Entry.comparingByValue())
                .ifPresent(e -> System.out.println("• Médico que mais atendeu: " + e.getKey() + " (" + e.getValue() + ")"));
        ...
        porEsp.entrySet().stream().max(Map.Entry.comparingByValue())
                .ifPresent(e -> System.out.println("• Especialidade mais procurada: " + e.getKey() + " (" + e.getValue() + ")"));
    }
    ...
    private void planosSaude() {
        ...
        if (pacientesEsp.isEmpty()) { System.out.println("Nenhum paciente com convênio."); return; }
        ...
        gh.getConsultas().stream()
                .filter(c -> c.getStatus() == StatusConsulta.REALIZADA && c.getPaciente() instanceof PacienteEspecial)
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.summingDouble(c -> {
                            ...
                            double pago = ((PacienteEspecial)c.getPaciente()).custoDaConsulta(c.getMedico());
                            return Math.max(0, cheio - pago);
                        })
                )).forEach((plano, valor) -> System.out.println("  " + plano + ": " + String.format("%.2f", valor)));
    }
